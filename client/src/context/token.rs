...
impl TokenContext {
    ...
    pub async fn new_token(rpc: &RpcClient, token_program: Option<Pubkey>) -> anyhow::Result<Self> {
        let authority = fund_account(rpc, None).await?;
    ...
    pub async fn create_ata_for(&self, rpc: &RpcClient, owner: &Keypair) -> anyhow::Result<Pubkey> {
        ...
        send_transaction(rpc, owner, &[owner], &[create_ata_instruction]).await?;
    ...
    pub async fn mint_to(
        ...
        send_transaction(rpc, owner, &[&self.mint_authority], &[mint_to]).await
    }

    pub fn get_balance_for(&self, rpc: &RpcClient, owner: &Pubkey) -> anyhow::Result<u64> {
        let ata = self.get_ata_for(owner);
        let account_data = rpc.get_account_data(&ata)?;
