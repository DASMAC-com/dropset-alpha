...
impl<'a> MarketChecker<'a> {
    ...
    /// Retrieves the market's [MarketHeaderView] and passes it to `f` for custom assertions.
    ...
    /// Retrieves the market's asks and passes them to `f` for custom assertions.
    pub fn asks(&self, f: impl FnOnce(Vec<OrderView>)) {
        let market = self.mollusk.view_market(self.market_ctx.market);
        f(market.asks);
    }
}
