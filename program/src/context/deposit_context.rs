...
/// The account context for the [`Deposit`] instruction, verifying token ownership, mint
...
impl<'a> DepositContext<'a> {
    ...
    pub unsafe fn load(accounts: &'a [AccountView]) -> Result<DepositContext<'a>, ProgramError> {
        let Deposit {
            event_authority,
            user,
            market_account,
